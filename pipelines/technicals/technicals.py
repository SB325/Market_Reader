...
class tech_ingest():
    ...
    def get_price_history(self,varargin):
        ...
        if len(varargin)==9:
            data = self.auth.get_request(url=self.resource_url + '/' + varargin[0] + '/pricehistory', \
                        params={ 
                ...
                )  #The number of periods to show in each candle
            
    def get_quotes(self,tickers: list):        
        delim = ','
        data = self.auth.get_request(url=self.resource_url + '/quotes', 
            params={'symbol': delim.join(tickers)}, 
        )
        if not data.ok:
            msg='tda_market_data: get_quotes data call failed'
            print(msg)
        else:
            print(['tda_market_data: Downloaded quotes for ' + varargin[0] + '.'])
        return data

    def get_quote(self,tick: str):
        # len(varargin) must be 2. varargin[0] is a ticker symbol        
        data = self.auth.get_request(url=self.resource_url + '/' + tick + '/quotes')
        ...
        if not data.ok:
            ...
            print(['tda_market_data: Downloaded quote for ' + tick + '.'])
        return data
